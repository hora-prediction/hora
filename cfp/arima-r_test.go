...
func TestInsert(t *testing.T) {
	...
	a.Insert(mondat.MonDatPoint{c, time.Unix(0, 0), 1000}) // should be dropped
	a.Insert(mondat.MonDatPoint{c, time.Unix(0, 0), 2000}) // should be dropped
	for _, v := range testdat {
		a.Insert(mondat.MonDatPoint{c, time.Unix(0, 0), v})
	}
	dat := a.GetData()
	for i, v := range dat {
		expected := mondat.MonDatPoint{c, time.Unix(0, 0), testdat[i]}
		if v != expected {
